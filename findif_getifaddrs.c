                 ...
                 char *ebuf)
{
	...
	printf("Using getifaddrs() \n "); //DEBUG
        ...
        for (cur = ifa; cur; cur = cur->ifa_next) {
                in_addr_t addr, mask;
		printf("IN LOOP FINDING INTERFACE \n "); //DEBUG
		...
		dlAddr = (const struct sockaddr_dl *)cur->ifa_addr;//DEBUG
                base = (const uint8_t *) &dlAddr->sdl_data[dlAddr->sdl_nlen];//DEBUG
